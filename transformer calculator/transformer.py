...
# *** EEE-3003. ELECTROMECHANICAL ENERGY CONVERSION GROUP PROJECT ***
# AHMAD ZAMEER NAZARI (220702706) & İSMET MERT ŞEN (210702011)
...
author2 = 'İsmet Mert ŞEN'
credits = 'PySimpleGUI, matplotlib, numpy draw.io, latex2image by joeraut, icon from thenounproject \
...
col_input_data = sg.Column([
    [sg.Push(), sg.Text('Power Rating: '),sg.Input(key = '-INP_S-', size=7), sg.Text('VA', s=5)],
    [sg.Push(), sg.Text('Primary Voltage: '), sg.Input(key = '-INP_V_P-', size=7), sg.Text('V', s=5)],
    [sg.Push(), sg.Text('Secondary Voltage: '), sg.Input(key = '-INP_V_S-', size=7), sg.Text('V', s=5)]
])

col_input_oc = sg.Column([
    [sg.Push(), sg.Image('images/V_OC.png'), sg.Input(key = '-INP_V_OC-', size=5), sg.Text('V', s=5)],
    [sg.Push(), sg.Image('images/I_OC.png'), sg.Input(key = '-INP_I_OC-', size=5), sg.Text('A', s=5)],
    [sg.Push(), sg.Image('images/P_OC.png'), sg.Input(key = '-INP_P_OC-', size=5), sg.Text('W', s=5)]
])

col_input_sc = sg.Column([
    [sg.Push(), sg.Image('images/V_SC.png'), sg.Input(key = '-INP_V_SC-', size=5), sg.Text('V', s=5)],
    [sg.Push(), sg.Image('images/I_SC.png'), sg.Input(key = '-INP_I_SC-', size=5), sg.Text('A', s=5)],
    [sg.Push(), sg.Image('images/P_SC.png'), sg.Input(key = '-INP_P_SC-', size=5), sg.Text('W', s=5)]
])
...
col_crd = [
    ...
    [sg.Sizer(50,50)],
    [sg.Text('CREDITS:', font=('Aptos', 9, 'bold'))],
    [sg.Text(credits, font=('Aptos', 9, 'italic'), justification='center')]
]
